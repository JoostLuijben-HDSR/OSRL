...
class VAE(nn.Module):
    ...
    def __init__(
        self, obs_dim, act_dim, hidden_size, latent_dim, act_lim, device="cpu"
    ):
    ...
    def decode(self, obs, z=None):
        if z is None:
            z = (
                torch.randn((obs.shape[0], self.latent_dim))
                .clamp(-0.5, 0.5)
                .to(self.device)
            )
    ...
    def decode_multiple(self, obs, z=None, num_decode=10):
        if z is None:
            z = (
                torch.randn((obs.shape[0], num_decode, self.latent_dim))
                .clamp(-0.5, 0.5)
                .to(self.device)
            )
