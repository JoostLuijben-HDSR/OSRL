...
def random_augmentation(
    ...
    for i, target in zip(idxes, sampled_cr):
        ...
        cost_ret += (
            target_cost_ret
            - cost_ret[0]
            + np.random.normal(loc=0, scale=cstd, size=cost_ret.shape)
        )
        rew_ret += (
            target_rew_ret
            - rew_ret[0]
            + np.random.normal(loc=0, scale=rstd, size=rew_ret.shape)
        )
        aug_trajs.append(associated_traj)
...
class SequenceDataset(IterableDataset):
    ...
    def __init__(
        ...
        if cost_sample:
            ...
            self.sample_prob = compute_sample_prob(
                self.dataset, self.pareto_frontier, 1
            )
        else:
